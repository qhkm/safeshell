...
get_latest_version() {
    curl -fsSL "https://api.github.com/repos/$REPO/releases/latest" 2>/dev/null | \
        grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/' || echo "v0.1.3"
}
...
main() {
    ...
    echo -e "  ${CYAN}safeshell config${NC}          - View/modify settings"
    echo -e "  ${CYAN}safeshell schedule enable${NC}   - Enable auto-cleanup"
    echo -e "  ${CYAN}safeshell upgrade${NC}         - Upgrade to latest version"
    echo ""
    echo -e "${BOLD}Let agents run freely. Everything is reversible.${NC}"
}
